...
export function useSpeechRecognition() {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      if (
        ...
        alert(
          "Microphone access was denied. Please enable microphone access in your browser settings to use voice input."
        );
      } else {
        // Other type of error
        alert(
          "Could not start recording. Please check your microphone or try a different browser."
        );
      }
    }
  };
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      const processingPromise = new Promise<string | null>((resolve) => {
      ...
      // Clean up tracks after stopping
      if (mediaRecorder.stream) {
        setTimeout(() => {
          mediaRecorder.stream.getTracks().forEach((track) => track.stop());
  ...
  const processSpeechToText = async (audioBlob: Blob): Promise<string | null> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Failed to transcribe speech");
      }
