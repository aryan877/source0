...
const SecureFileDisplay = memo(
  ({
    ...
    if (isImage && mimeType.startsWith("image/") && (!url || imageError)) {
      return (
        <div
          className={`mb-3 overflow-hidden rounded-lg border border-divider bg-content1 ${
            isSmall ? "w-32" : "max-w-sm"
          } ${className ?? ""}`}
        >
          <div
            className={`flex w-full items-center justify-center bg-content2 ${
              isSmall ? "h-24" : "h-32"
            }`}
          >
            <div className="flex flex-col items-center gap-2 text-default-400">
              <PhotoIcon className={isSmall ? "h-6 w-6" : "h-8 w-8"} />
    ...
    if (!url) {
      return (
        <div
          className={`mb-3 rounded-lg border border-danger/30 bg-danger/5 p-2 ${className ?? ""}`}
        >
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <span className="block truncate text-xs font-medium text-danger">
                File unavailable
              </span>
    ...
    return (
      <div
        className={`mb-3 rounded-lg bg-default-100 px-3 py-2 dark:bg-default-50 ${className ?? ""}`}
      >
