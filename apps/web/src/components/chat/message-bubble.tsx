...
const CodeBlock = memo(({ children, className }: CodeBlockProps) => {
  ...
  const lineNumbers = useMemo(() => {
    ...
    return Array.from({ length: totalLines }, (_, index) => (
      <div
        key={index}
        className="h-6 select-none px-2 text-right font-mono text-xs leading-6 text-default-500"
      >
        {index + 1}
      </div>
    ));
  ...
  return (
    <div className="my-3 overflow-hidden rounded-md border border-divider bg-content1">
